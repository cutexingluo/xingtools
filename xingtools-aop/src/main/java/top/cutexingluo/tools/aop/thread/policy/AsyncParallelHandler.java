 ...
 * @author XingTian
 * @version 1.0.0
 * @date 2023/10/2 15:17
 * @since 1.0.2
 */
@Data
@AllArgsConstructor
public class AsyncParallelHandler implements ThreadAopHandler {
    public static final ThreadPolicy policy = ThreadPolicy.AsyncParallel;
    ...
    public Object runInMainAop(ProceedingJoinPoint joinPoint, MainThread mainThread) throws Exception {
        ...
        try {
            ...
            if (mainThread.startTime() == ThreadTimePolicy.AsyncAfterMain) {
                CompletableFuture<Void> allOfTask = (CompletableFuture<Void>) map.get(name + ":allTasks");
                allOfTask.get(); // 运行完后执行
                if (isGroup) {
...
//        if (rollbackPolicy==RollbackPolicy.Group && rollBackFlag.get()) {  // 后续无需执行
//            return null;
//        }
...
//                    if (rollBackFlag.get()) { //先自行回滚，//注释后面统一回滚
//                        meta.rollback();
//                    }
                } else {
                    meta.commit();
